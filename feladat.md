...
- Az alkalmazást **Laravel 11** keretrendszerben, **SQLite** adatbázis használatával kell megvalósítani!
  - Ha az ORM-et szabályosan használod, akkor egyébként nem számít, hogy milyen adatbázis fut a háttérben a fejlesztés során, de mi a teszteléshez és értékeléshez _SQLite_ adatbázist fogunk használni.
...
<a
  class="..."
  href="{{ route('match.attack', ['id' => $match->id, 'attackType' => 'melee']) }}"
  >Melee</a
>
<a
  class="..."
  href="{{ route('match.attack', ['id' => $match->id, 'attackType' => 'ranged']) }}"
  >Ranged</a
>
<a
  class="..."
  href="{{ route('match.attack', ['id' => $match->id, 'attackType' => 'special']) }}"
  >Special</a
>
```
